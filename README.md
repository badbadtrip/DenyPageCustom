...
### 3. (опционально) Вход по QR без ввода пароля — [lampa-nextgen-tg-bot](https://github.com/badbadtrip/lampa-nextgen-tg-bot)

Если рядом установлен модуль `TelegramBot` из репозитория **lampa-nextgen-tg-bot** (тот, что выдаёт токены по заявкам), deny.js **автоматически** обнаруживает его HTTP-эндпоинты `/tgbot/qr/*` и включает пассивный вход: пользователь сканирует QR на этой странице → в открывшемся боте нажимает «✅ Подтвердить вход» → deny.js сам подставляет уже выданный ему токен и логинится, без набора пароля на пульте/клавиатуре. Работает только для пользователя, у которого уже есть активный (не истёкший) доступ, выданный ботом — это способ входа в существующий аккаунт на новом устройстве, а не способ его получить.

Если модуль `TelegramBot` не установлен или эндпоинт недоступен — QR молча остаётся обычной ссылкой на бота (`tg_target`), поведение из п.2 не меняется.

**Обязательно:** пути `/tgbot/qr/start` и `/tgbot/qr/status` должны быть доступны неавторизованному посетителю — добавьте их в `accsdb.whitepattern` (см. [Поле accsdb](#1-включить-accsdb-в-основном-initconf-сервера)), иначе сам accsdb будет их блокировать раньше, чем запрос дойдёт до контроллера:
